...
| `HPY-PYPY-BRIDGE` | PyPy bundled HPy bridge | Hosted job `102193146755` passes handwritten and generated constant-only oracles plus Fibonacci import, then captures 65 native frames after signal 11 in isolated `fibonacci-semantics`; top frame is `pypy_g_HPy_Length` | Reduced report in `audits/prd5-pypy-bridge-upstream-report.md`; handwritten `HPyFunc_KEYWORDS` isolation and filing authorization pending | unsupported early warning |
...
trace rooted at PyPy's `HPy_Length` implementation. A handwritten
`HPyFunc_KEYWORDS` method must now isolate the no-keyword positional-call path
before filing. Run `34030366000` reconfirmed GraalPy's loader gap with an unchanged handwritten
